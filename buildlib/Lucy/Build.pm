...
my $XS_SOURCE_DIR        = 'xs';
my $AUTOBIND_PM_PATH     = catfile(qw( lib Search Kino03 KinoSearch Autobinding.pm ));
my $AUTOBIND_XS_PATH     = catfile(qw( lib Search Kino03 KinoSearch Autobinding.xs ));
...
sub use_kinosearch_mode {
    $XS_FILEPATH = 'Search/Kino03/KinoSearch.xs';
...
sub ACTION_compile_custom_xs {
    ...
    my $cbuilder = Lucy::Build::CBuilder->new;
    my $archdir = catdir( $self->blib, 'arch', 'auto', 'Search', 'Kino03', 'KinoSearch' );
    ...
    # .xs => .c
    my $ks_c_file = 'Search/Kino03/KinoSearch.c';
    ...
    my $ks_o_file = "Search/Kino03/KinoSearch$Config{_o}";
...
sub _gen_pause_exclusion_list {
    ...
    require 'lib/Search/Kino03/KinoSearch/Redacted.pm';
    my @redacted = map {
        my @parts = split( /\W+/, $_ );
        catfile( 'lib', @parts ) . '.pm'
    } Search::Kino03::KinoSearch::Redacted->list;
