...
sub new {
    ...
    my $dunked_type = Search::Kino03::KinoSearch::FieldType::FullTextType->new(
        analyzer      => $tokenizer,
        highlightable => 1,
        boost         => 0.1,
    );
...
use base qw( Search::Kino03::KinoSearch::Highlight::Highlighter );
...
my $hl   = Search::Kino03::KinoSearch::Highlight::Highlighter->new(
    searchable     => $searcher,
    query          => $q,
    field          => 'content',
    excerpt_length => 3,
...
my $target = Search::Kino03::KinoSearch::Util::ViewCharBuf->_new("");
...
$hl = Search::Kino03::KinoSearch::Highlight::Highlighter->new(
    searchable     => $searcher,
    query          => $q,
    field          => 'content',
    excerpt_length => 6,
...
like(
    Search::Kino03::KinoSearch::Highlight::Highlighter->new(
        searchable => $searcher,
        query      => $q,
        field      => 'content',
...
sub make_cb {
    return Search::Kino03::KinoSearch::Util::CharBuf->new(shift);
}

sub make_heat_map {
    return Search::Kino03::KinoSearch::Highlight::HeatMap->new( spans => make_spans(@_) );
}

sub make_span {
    return Search::Kino03::KinoSearch::Search::Span->new(
        offset => $_[0],
        length => $_[1],
        weight => $_[2],
    );
...
sub make_spans {
    my $spans = Search::Kino03::KinoSearch::Util::VArray->new( capacity => scalar @_ );
    for my $span_spec (@_) {
