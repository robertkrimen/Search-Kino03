...
isa_ok( $and_scorer, "Search::Kino03::KinoSearch::Search::ANDScorer", "make_matcher" );

my $term_scorer = $one_child->make_compiler( searchable => $searcher )
    ->make_matcher( reader => $reader, need_score => 0 );
isa_ok(
    $term_scorer,
    "Search::Kino03::KinoSearch::Search::TermScorer",
    "make_matcher compiles to child's scorer if there's only one child"
);

my $hopeless_query = Search::Kino03::KinoSearch::Search::TermQuery->new(
    field => 'nyet',
    term  => 'nein',
);
