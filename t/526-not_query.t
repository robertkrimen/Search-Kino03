...
my $b_query = Search::Kino03::KinoSearch::Search::TermQuery->new(
    field => 'content',
    term  => 'b'
);
my $c_query = Search::Kino03::KinoSearch::Search::TermQuery->new(
    field => 'content',
    term  => 'c'
);
my $not_b_query
    = Search::Kino03::KinoSearch::Search::NOTQuery->new( negated_query => $b_query );
my $not_c_query
    = Search::Kino03::KinoSearch::Search::NOTQuery->new( negated_query => $c_query );
...
for my $num_negated ( 1 .. 26 ) {
    ...
    my $mock_scorer = Search::Kino03::KSx::Search::MockScorer->new(
        doc_ids => \@mock_ids,
        scores  => [ (1) x scalar @mock_ids ],
    );
    my $not_scorer = Search::Kino03::KinoSearch::Search::NOTScorer->new(
        doc_max         => $reader->doc_max,
        negated_matcher => $mock_scorer,
    );
    my $bit_vec = Search::Kino03::KinoSearch::Util::BitVector->new( capacity => 30 );
