...
sub test_search {
    ...
    if (@$dels) {
        my $bit_vec
            = Search::Kino03::KinoSearch::Util::BitVector->new( capacity => $dels->[-1] + 1 );
        $bit_vec->set($_) for @$dels;
        $del_enum = Search::Kino03::KinoSearch::Search::BitVecMatcher->new(
            bit_vector => $bit_vec );
    }
    ...
    my $collector = Search::Kino03::KinoSearch::Search::HitCollector::SortCollector->new(
        wanted => 100 );
    $matcher->collect(
        %Search::Kino03::KinoSearch::Search::Matcher::collect_PARAMS,
        collector => $collector,
        deletions => $del_enum,
        %args,
    );
