...
package Search::Kino03::KSx::Search::Filter;
BEGIN { our @ISA = qw( Search::Kino03::KinoSearch::Search::Query ) }
use Carp;
...
sub new {
    ...
    confess("required parameter query is not a Search::Kino03::KinoSearch::Search::Query")
        unless ( blessed($query)
        && $query->isa('Search::Kino03::KinoSearch::Search::Query') );
...
sub make_compiler {
    my $self = shift;
    return Search::Kino03::KSx::Search::FilterCompiler->new( @_, parent => $self );
}
...
sub _bits {
    ...
    if ( !defined $cached_bits ) {
        $cached_bits = Search::Kino03::KinoSearch::Util::BitVector->new(
            capacity => $seg_reader->doc_max + 1 );
        $self->_store_cached_bits( $seg_reader, $cached_bits );

        my $collector = Search::Kino03::KinoSearch::Search::HitCollector::BitCollector->new(
            bit_vector => $cached_bits );

        my $polyreader = Search::Kino03::KinoSearch::Index::PolyReader->new(
            schema      => $seg_reader->get_schema,
            folder      => $seg_reader->get_folder,
            snapshot    => $seg_reader->get_snapshot,
            sub_readers => [$seg_reader],
        );
        my $searcher = Search::Kino03::KinoSearch::Searcher->new( index => $polyreader );
...
sub _store_cached_bits {
    ...
    $cached_bits{$$self}{ $seg_reader->hash_code } = $pair;
}
...
sub _fetch_cached_bits {
    ...
    # Fetch.
    my $pair = $cached_bits->{ $seg_reader->hash_code };
...
package Search::Kino03::KSx::Search::FilterCompiler;
BEGIN { our @ISA = qw( Search::Kino03::KinoSearch::Search::Compiler ) }
...
sub make_matcher {
    ...
    return Search::Kino03::KSx::Search::FilterScorer->new(
        bits    => $bits,
        doc_max => $seg_reader->doc_max,
    );
}

package Search::Kino03::KSx::Search::FilterScorer;
BEGIN { our @ISA = qw( Search::Kino03::KinoSearch::Search::Matcher ) }
...
{   "Search::Kino03::KSx::Search::FilterScorer" => {
...
Search::Kino03::KSx::Search::Filter - Build a caching filter based on results of a Query.

=head1 SYNOPSIS

    my %category_filters;
    for my $category (qw( sweet sour salty bitter )) {
        my $cat_query = Search::Kino03::KinoSearch::Search::TermQuery->new(
            field => 'category',
            term  => $category,
        );
        $category_filters{$category} = Search::Kino03::KSx::Search::Filter->new( 
            query => $cat_query, 
        );
    }
...
A Filter is a L<Search::Kino03::KinoSearch::Search::Query> subclass that can be used to filter
...
=head2 new
    ...
    my $filter = Search::Kino03::KSx::Search::Filter->new(
        query => $query;
    );

Constructor.  Takes one hash-style parameter, C<query>, which must be an object
belonging to a subclass of L<Search::Kino03::KinoSearch::Search::Query>.

=head1 BUGS

Filters do not cache when used in a search cluster with Search::Kino03::KSx::Remote's
