...
package Search::Kino03::KSx::Simple;
use Carp;
...
sub new {
    ...
    # Get type and schema.
    my $analyzer
        = Search::Kino03::KinoSearch::Analysis::PolyAnalyzer->new( language => $language );
    $self->{type}
        = Search::Kino03::KinoSearch::FieldType::FullTextType->new( analyzer => $analyzer, );
    my $schema = $self->{schema} = Search::Kino03::KinoSearch::Schema->new;
...
sub _lazily_create_indexer {
    ...
    if ( !defined $self->{indexer} ) {
        $self->{indexer} = Search::Kino03::KinoSearch::Indexer->new(
            schema => $self->{schema},
            index  => $self->{path},
        );
    }
...
sub search {
    ...
    if ( !defined $self->{searcher} ) {
        $self->{searcher}
            = Search::Kino03::KinoSearch::Searcher->new( index => $self->{path} );
    }
...
Search::Kino03::KSx::Simple - Basic search engine.

=head1 SYNOPSIS

First, build an index of your documents.

    my $index = Search::Kino03::KSx::Simple->new(
        path     => '/path/to/index/'
        language => 'en',
    );
...
Search::Kino03::KSx::Simple is a stripped-down interface for the L<KinoSearch> search
engine library.  
...
Create a Search::Kino03::KSx::Simple object, which can be used for both indexing and
