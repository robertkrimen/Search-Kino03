...
PolyLex_reset(PolyLexicon *self)
{
    ...
    u32_t   num_segs     = VA_Get_Size(seg_lexicons);
...
PolyLex_seek(PolyLexicon *self, Obj *target)
{
    ...
    do {
        if (self->term) {
            const i32_t comparison = Obj_Compare_To(self->term, target);
