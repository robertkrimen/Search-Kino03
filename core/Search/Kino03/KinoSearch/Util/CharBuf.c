...
{
    /* Derive. */
    self->ptr = MALLOCATE(size + 1, char);
...
{
    kino_MemMan_wrapped_free(self->ptr);
...
SI_maybe_grow(CharBuf *self, size_t new_size) 
{
    ...
    self->ptr = REALLOCATE(self->ptr, (new_size + 1), char);
    self->cap = new_size + 1;
}
...
CB_swap_chars(CharBuf *self, u32_t match, u32_t replacement)
{
    ...
    if (match > 127) { 
        THROW("match point too high: %u32", match);
    }
    else if (replacement > 127) { 
        THROW("replacement code point too high: %u32", replacement);
    }
    else {
        char *ptr = self->ptr;
...
void
CB_copy_str(CharBuf *self, const char* ptr, size_t size) 
{
    if (!StrHelp_utf8_valid(ptr, size))
        S_die_invalid_utf8(ptr, size);
...
{
    target->vtable = (VTable*)&ZOMBIECHARBUF;
...
ViewCB_grow(ViewCharBuf *self, size_t size)
{
    UNUSED_VAR(self);
    UNUSED_VAR(size);
    THROW("Can't grow a ViewCharBuf ('%o')", self);
}
...
{
    THROW("Can't destroy a ZombieCharBuf ('%o')", self);
}
